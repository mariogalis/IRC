...
int Server::Try_01()
{
    ...
    while (true) 
    {
        ...
        {
            // if (clientes.size() == 0)
            //     std::cout << "Cliente size = " << clientes.size() << std::endl;
            if (_sockets[i].revents & POLLIN) 
            {
                if (_sockets[i].fd == server_socket)
                {
                    ...
                    while (std::getline(iss, token, '\n'))
                        tokens.push_back(token);
                    for (size_t j = 0; i < tokens.size(); ++i)
                    {
                ...
                else
                {
                    for (size_t j = 0; j < clientes.size(); ++j) 
                    {
                        ...
                        if (_sockets[j + 1].revents & POLLIN) 
                        {
                            int bytes_received = recv(client_socket, buffer, BUFFER_SIZE, 0);
                            if (find_ClientData_Socket(client_socket) != clientes.end())
                            {
                                if (bytes_received <= 0) 
                                {
                                    std::cerr << "ANTES == " << clientes.size() << std::endl << std::endl;
                                    clientes.erase(find_ClientData_Socket(client_socket)); // Eliminar el elemento del vector
                                    _sockets.erase(_sockets.begin() + i);
                                    close(client_socket);
                                    std::cerr << "DESPUES == "<< clientes.size() << std::endl << std::endl;
                                    break;
                                }
